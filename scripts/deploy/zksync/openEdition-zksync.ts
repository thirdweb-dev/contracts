...
async function main() {
  ...
  const artifact: ZkSyncArtifact = (await hre.artifacts.readArtifact("OpenEditionERC721")) as ZkSyncArtifact;
  const contractFactory = new ContractFactory(artifact.abi, artifact.bytecode, wallet, "create");

  const contract = await contractFactory.deploy();
  await contract.deployed();

  console.log("Deployed OpenEditionERC721 \n: ", contract.address);

  console.log("\n");

  console.log("Verifying contract.");
  await verify(contract.address, "contracts/prebuilts/open-edition/OpenEditionERC721.sol:OpenEditionERC721", []);
}
