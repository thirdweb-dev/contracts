...
async function main() {
  ...
  const artifact: ZkSyncArtifact = (await hre.artifacts.readArtifact("DropERC1155")) as ZkSyncArtifact;
  const contractFactory = new ContractFactory(artifact.abi, artifact.bytecode, wallet, "create");

  const contract = await contractFactory.deploy();
  await contract.deployed();

  console.log("Deployed DropERC1155 \n: ", contract.address);

  console.log("\n");

  console.log("Verifying contract.");
  await verify(contract.address, "contracts/prebuilts/drop/DropERC1155.sol:DropERC1155", []);
}
