...
async function main() {
  ...
  const artifact: ZkSyncArtifact = (await hre.artifacts.readArtifact("VoteERC20")) as ZkSyncArtifact;
  const contractFactory = new ContractFactory(artifact.abi, artifact.bytecode, wallet, "create");

  const contract = await contractFactory.deploy();
  await contract.deployed();

  console.log("Deployed VoteERC20 \n: ", contract.address);

  console.log("\n");

  console.log("Verifying contract.");
  await verify(contract.address, "contracts/prebuilts/vote/VoteERC20.sol:VoteERC20", []);
}
