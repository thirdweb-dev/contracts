...
async function main() {
  ...
  const artifact: ZkSyncArtifact = (await hre.artifacts.readArtifact("LoyaltyCard")) as ZkSyncArtifact;
  const contractFactory = new ContractFactory(artifact.abi, artifact.bytecode, wallet, "create");

  const contract = await contractFactory.deploy();
  await contract.deployed();

  console.log("Deployed LoyaltyCard \n: ", contract.address);

  console.log("\n");

  console.log("Verifying contract.");
  await verify(contract.address, "contracts/prebuilts/loyalty/LoyaltyCard.sol:LoyaltyCard", []);
}
