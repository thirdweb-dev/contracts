...
async function createPack(
  ...
  const createPackTx = await rewardsContract
    .connect(caller)
    .createPackAtomic(rewardURIs, rewardSupplies, packURI, openStart, openEnd);
...
createPack(packURI, rewardURIs, rewardSupplies, openStartAndEnd, openStartAndEnd)
  .then(() => process.exit(0))
  .catch(err => {
    console.error(err);
    process.exit(1);
