...
```solidity
function claim(address _receiver, uint256 _quantity, address _currency, uint256 _pricePerToken, IDropUpdated.AllowlistProof _allowlistProof, bytes _data) external payable
```
...
| _allowlistProof | IDropUpdated.AllowlistProof | undefined
...
```solidity
function verifyClaimMerkleProof(uint256 _conditionId, address _claimer, uint256 _tokenId, uint256 _quantity, IDropUpdated.AllowlistProof _allowlistProof) external view returns (bool validMerkleProof, uint256 merkleProofIndex)
```
