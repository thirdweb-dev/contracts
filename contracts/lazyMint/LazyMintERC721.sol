...
// TODO: Combine ERC721Enumerable, ERC721Burnable, ERC721Pausable
    ...
    Multicall
{
    ...
    PublicMintConditions public mintConditions;
    ...
    // function lazyMintWithURI(string[] calldata _uris) external {
    //     require(hasRole(MINTER_ROLE, _msgSender()), "only minter");
    //     require((nextTokenIdToMint + _uris.length) <= maxTotalSupply, "exceed maxTotalSupply");
    ...
    //     require(hasRole(MINTER_ROLE, _msgSender()), "only minter");
    //     require((nextTokenIdToMint + amount) <= maxTotalSupply, "exceed maxTotalSupply");
    //     nextTokenIdToMint += amount;
    // }
    ...
    function claim(uint256 quantity, bytes32[] calldata proofs) external payable nonReentrant {
        ...
        MintCondition memory currentMintCondition = mintConditions.mintConditionAtIndex[conditionIndex];

        require(quantity > 0 
            && quantity <= currentMintCondition.quantityLimitPerTransaction, 
            "invalid quantity claimed"
        );
        require(nextTokenIdToClaim + quantity <= maxTotalSupply, "exceed max supply limit");
        require(nextTokenIdToClaim + quantity <= nextTokenIdToMint, "cannot claim unminted token");
        ...
        mintConditions.mintConditionAtIndex[conditionIndex].currentMintSupply += quantity;
        ...
        uint256 startMintTokenId = nextTokenIdToClaim;
        for (uint256 i = 0; i < quantity; i++) {
            _safeMint(_msgSender(), nextTokenIdToClaim);
