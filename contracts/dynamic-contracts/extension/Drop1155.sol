...
import "../../extension/interface/IDrop1155.sol";
import "../../lib/MerkleProof.sol";

library Drop1155Storage {
    bytes32 public constant DROP_1155_STORAGE_POSITION = keccak256("drop.1155.storage");

    struct Data {
        /// @dev The active conditions for claiming tokens.
        mapping(uint256 => IDrop1155.ClaimConditionList) claimCondition;
    }

    function drop1155Storage() internal pure returns (Data storage drop1155Data) {
        bytes32 position = DROP_1155_STORAGE_POSITION;
        assembly {
            drop1155Data.slot := position
        }
    }
}

abstract contract Drop1155 is IDrop1155 {
    function claimCondition(uint256 _tokenId) public view returns (uint256, uint256) {
        Drop1155Storage.Data storage data = Drop1155Storage.drop1155Storage();
        return (data.claimCondition[_tokenId].currentStartId, data.claimCondition[_tokenId].count);
    }
    ...
    function claim(
        ...
        Drop1155Storage.Data storage data = Drop1155Storage.drop1155Storage();
        data.claimCondition[_tokenId].conditions[activeConditionId].supplyClaimed += _quantity;
        data.claimCondition[_tokenId].supplyClaimedByWallet[activeConditionId][_dropMsgSender()] += _quantity;
    ...
    function setClaimConditions(
        ...
        Drop1155Storage.Data storage data = Drop1155Storage.drop1155Storage();
        ClaimConditionList storage conditionList = data.claimCondition[_tokenId];
    ...
    function verifyClaim(
        ...
        Drop1155Storage.Data storage data = Drop1155Storage.drop1155Storage();
        ClaimCondition memory currentClaimPhase = data.claimCondition[_tokenId].conditions[_conditionId];
        ...
        uint256 supplyClaimedByWallet = data.claimCondition[_tokenId].supplyClaimedByWallet[_conditionId][_claimer];
    ...
    {
        Drop1155Storage.Data storage data = Drop1155Storage.drop1155Storage();
        condition = data.claimCondition[_tokenId].conditions[_conditionId];
    }
    ...
    function getSupplyClaimedByWallet(
        ...
        Drop1155Storage.Data storage data = Drop1155Storage.drop1155Storage();
        supplyClaimedByWallet = data.claimCondition[_tokenId].supplyClaimedByWallet[_conditionId][_claimer];
    }
