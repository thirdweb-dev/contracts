...
contract ExtensionRegistryTest is BaseTest, IExtension {
    ...
    /*///////////////////////////////////////////////////////////////
                Setting extensions for contract type
    //////////////////////////////////////////////////////////////*/

    function test_state_setExtensionsForContractType() external {
        uint256 len = 3;
        ...
        // Add extensionn names for contract type.
        vm.prank(registryDeployer);
        extensionRegistry.setExtensionsForContractType(contractType, extensionNames);

        string[] memory allContractTypes = extensionRegistry.getAllContractTypes();
        assertEq(allContractTypes.length, 1);
        assertEq(allContractTypes[0], contractType);

        Extension[] memory contractTypeExtensions = extensionRegistry.getLatestExtensionsForContractType(contractType);

        for (uint256 i = 0; i < len; i += 1) {
            assertEq(contractTypeExtensions[i].metadata.implementation, extensions[i].metadata.implementation);
            assertEq(contractTypeExtensions[i].metadata.name, extensions[i].metadata.name);
            assertEq(contractTypeExtensions[i].metadata.metadataURI, extensions[i].metadata.metadataURI);
            uint256 fnsLen = extensions[i].functions.length;
            assertEq(fnsLen, contractTypeExtensions[i].functions.length);
            for (uint256 j = 0; j < fnsLen; j += 1) {
                assertEq(
                    extensions[i].functions[j].functionSelector,
                    contractTypeExtensions[i].functions[j].functionSelector
                );
                assertEq(
                    extensions[i].functions[j].functionSignature,
                    contractTypeExtensions[i].functions[j].functionSignature
                );
            }
        }

        // Update the extensions for contract type.
        Extension memory extension1;
        ...
        string[] memory newExtensionNames = new string[](len + 1);
        for (uint256 i = 0; i < len; i += 1) {
            newExtensionNames[i] = extensionNames[i];
        }
        newExtensionNames[len] = extension1.metadata.name;

        vm.prank(registryDeployer);
        extensionRegistry.setExtensionsForContractType(contractType, newExtensionNames);

        allContractTypes = extensionRegistry.getAllContractTypes();
        assertEq(allContractTypes.length, 1);
        assertEq(allContractTypes[0], contractType);

        contractTypeExtensions = extensionRegistry.getLatestExtensionsForContractType(contractType);
        assertEq(contractTypeExtensions.length, 4);

        assertEq(contractTypeExtensions[3].metadata.implementation, extension1.metadata.implementation);
        assertEq(contractTypeExtensions[3].metadata.name, extension1.metadata.name);
        assertEq(contractTypeExtensions[3].metadata.metadataURI, extension1.metadata.metadataURI);
        uint256 fnsLen = extension1.functions.length;
        assertEq(fnsLen, contractTypeExtensions[3].functions.length);
        for (uint256 j = 0; j < fnsLen; j += 1) {
            assertEq(extension1.functions[j].functionSelector, contractTypeExtensions[3].functions[j].functionSelector);
            assertEq(
                extension1.functions[j].functionSignature,
                contractTypeExtensions[3].functions[j].functionSignature
            );
        }
    }

    function test_revert_setExtensionsForContractType_unauthorizedCaller() external {
        Extension memory extension1;
    ...
    function test_revert_setExtensionsForContractType_emptycontractType() external {
        Extension memory extension1;
    ...
    function test_revert_setExtensionsForContractType_emptyExtensions() external {
        string memory contractType = "ExampleContract";
    ...
    function test_revert_setExtensionsForContractType_addingNonexistentExtension() external {
        string memory contractType = "ExampleContract";
        string[] memory extensionNames = new string[](1);
    ...
    /*///////////////////////////////////////////////////////////////
                Contract registering with a contract type
    //////////////////////////////////////////////////////////////*/

    address private router;
    string private contractType = "ExampleContract";

    function _setUp_registerContract() internal {
        router = address(0x1234);

        uint256 len = 3;

        string[] memory extensionNames = new string[](len);
        contractType = "ExampleContract";
    ...
    function test_state_registerContract() external {
        _setUp_registerContract();

        // Contract registers with contract type.
        vm.prank(router);
        extensionRegistry.registerContract(contractType);

        Extension[] memory getAllExtensions = extensionRegistry.getAllDefaultExtensionsForContract(router);
        ...
        for (uint256 i = 0; i < len; i += 1) {
            ...
            for (uint256 j = 0; j < fnsLen; j += 1) {
                ...
                ExtensionMetadata memory metadata = extensionRegistry.getExtensionForContractFunction(
                    extensions[i].functions[j].functionSelector,
                    router
                );
            ...
            // getExtension
            Extension memory extension = extensionRegistry.getDefaultExtensionForContract(
                extensions[i].metadata.name,
                router
            );
            assertEq(extension.metadata.implementation, extensions[i].metadata.implementation);
            assertEq(extension.metadata.name, extensions[i].metadata.name);
    ...
    function test_revert_registerContract_nonexistentContractType() external {
        router = address(0x1234);

        // Contract registers with contract type when contract type does not exist.
        vm.prank(router);
        vm.expectRevert("ExtensionRegistryState: contract type does not exist.");
        extensionRegistry.registerContract(contractType);
    }

    function test_revert_registerContract_routerAlreadyRegistered() external {
        _setUp_registerContract();
        ...
        vm.prank(router);
        vm.expectRevert("ExtensionRegistryState: contract already registered.");
        extensionRegistry.registerContract(contractType);
    }
    ...
    function test_state_setExtensionsForContractTypeWithSig() external {
        uint256 len = 3;
