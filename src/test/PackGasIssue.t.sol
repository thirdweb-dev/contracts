...
contract PackGasIssueTest is BaseTest {
    Pack internal pack;
    ...
    function setUp() public override {
        ...
        for(uint256 i = 0; i < 13; i++) {
            assets1155.push(new MockERC1155());

            for(uint256 j = 0; j < 10; j++) {
                packContents.push(
                    ITokenBundle.Token({
                        assetContract: address(assets1155[i]),
                        tokenType: ITokenBundle.TokenType.ERC1155,
                        tokenId: j,
                        totalAmount: 1
                    })
                ...
                assets1155[i].mint(address(tokenOwner), j, 1);
            }
            ...
            for(uint256 j = 10; j < 20; j++) {
                ...
                assets1155[i].mint(address(tokenOwner), j, 5);
            }

            for(uint256 j = 20; j < 30; j++) {
                packContents.push(
                    ITokenBundle.Token({
                        assetContract: address(assets1155[i]),
                        tokenType: ITokenBundle.TokenType.ERC1155,
                ...
                assets1155[i].mint(address(tokenOwner), j, 10);
            }

            tokenOwner.setApprovalForAllERC1155(address(assets1155[i]), address(pack), true);
        }
