...
describe("Mint tokens with a valid mint request", function () {
  ...
  describe("Revert cases", function () {
    it("Should revert if caller does not have minter role", async () => {
      await expect(sigMint721.connect(accountWithoutMinterRole).mintTo(nftReceiver.address, uri)).to.be.revertedWith(
        "not minter.",
      );
    });
  });
  ...
  describe("Events", function () {
    it("Should emit TokensMintedByMinter with mint information", async () => {
      ...
      await expect(sigMint721.connect(protocolAdmin).mintTo(nftReceiver.address, uri))
