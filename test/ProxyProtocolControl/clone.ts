...
describe("Deploy proxies for ProtocolControl using ControlDeployer", function () {
  ...
  it("Should prevent initializing a clone after deployment", async () => {
    await expect(
      clone_1.connect(protocolProvider).initialize(registry.address, protocolProvider.address, clone_1_URI),
    ).to.be.revertedWith("Initializable: contract is already initialized");

    await expect(
      clone_2.connect(protocolProvider).initialize(registry.address, protocolProvider.address, clone_1_URI),
    ).to.be.revertedWith("Initializable: contract is already initialized");
  });
