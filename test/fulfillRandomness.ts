...
import { Signer, Contract, ContractFactory, BytesLike, Bytes, BigNumber } from "ethers";
...
describe("Testing openPack", function() {
  ...
  describe("Revert cases", function() {
    ...
    it("Should revert if caller is not VRF coordinator", async () => {
      await expect(pack.connect(protocolAdmin).rawFulfillRandomness(requestId, randomNumber))
        .to.be.revertedWith("Only VRFCoordinator can fulfill")
    })
  })
    ...
    it("Should emit PackOpenFulfilled", async () => {
      ...
      const fulfillRandomnessPromise = new Promise((resolve, reject) => {
        pack.on("PackOpenFulfilled", async (_packId, _opener, _requestId, _source, _rewardId) => {
          ...
          expect(expectedRewardIds.includes(
            parseInt(_rewardId.toString())
          )).to.equal(true)
