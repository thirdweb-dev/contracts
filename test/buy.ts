...
describe("Buy packs using Market.sol", function() {
  ...
  describe("Revert cases", function() {
    it("Should revert if no quantity of tokens are bought", async () => {
      await expect(market.connect(fan).buy(expectedListingId, 0))
    ...
    it("Should revert if more tokens than are listed, are bought", async () => {
      await expect(market.connect(fan).buy(expectedListingId, expectedPackSupply + 1))
        .to.be.revertedWith("Market: must buy an appropriate amount of tokens.")
    })

    it("Should revert is Market is not approved to spend the required amount of currency", async () => {
      await expect(market.connect(fan).buy(expectedListingId, quantityToBuy))
        .to.be.revertedWith("Market: must approve Market to transfer price to pay.")
    })
  })
  ...
  describe("Balances", function() {
    it("Should transfer the relevant amounts of the listed tokens to the buyer", async () => {
      ...
      expect(await pack.balanceOf(market.address, expectedPackId)).to.equal(BigNumber.from(expectedPackSupply).sub(quantityToBuy))
