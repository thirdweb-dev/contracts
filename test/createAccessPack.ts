...
describe("Create a pack with rewards in a single tx", function() {
  ...
  describe("Revert", function() {
    ...
    it("Should revert if unequal number of URIs and supplies are provided", async () => {
        await expect(accessNft.connect(creator).createAccessPack(
          pack.address,
          rewardURIs.slice(1),
          accessURIs,
          rewardSupplies,
    ...
    it("Should revert if caller does not have MINTER_ROLE", async () => {
      await expect(accessNft.connect(relayer).createAccessPack(
        pack.address,
        rewardURIs,
        accessURIs,
        rewardSupplies,
  ...
  describe("Events", function() {
    ...
    it("Should emit AccessNFTsCreated", async () => {
      ...
      await accessNft.connect(creator).createAccessPack(
        pack.address,
        rewardURIs,
        accessURIs,
        rewardSupplies,
  ...
  describe("Balances", async () => {
    ...
    beforeEach(async () => {
      ...
      packId = parseInt(
        ( await pack.nextTokenId() ).toString()
      );
