...
describe("Bid: Auction Listing", function () {
  ...
  describe("Events", function() {
    ...
    it("Should not emit NewBid if incoming bid is not the new highest bid, and is below buyout price", async () => {
      ...
      await marketv2.connect(buyer).offer(listingId, quantityWanted, highOfferAmount)

      const lowOfferAmount = reservePricePerToken.mul(quantityToList);

      const txReceipt = await marketv2.connect(relayer).offer(listingId, quantityWanted, lowOfferAmount)
        .then(tx => tx.wait());
  ...
  describe("Balances", async () => {
    ...
    it("Should payout the previous bidder, if new bid is higher than the previous bid", async () => {
      ...
      const prevBuyerBalBefore: BigNumber = await coin.balanceOf(relayer.address)
      const buyerBalBefore: BigNumber = await coin.balanceOf(buyer.address)
      const marketBalBefore: BigNumber = await coin.balanceOf(marketv2.address);
      
      await marketv2.connect(relayer).offer(listingId, quantityWanted, lowOfferAmount);
      await marketv2.connect(buyer).offer(listingId, quantityWanted, highOfferAmount);
      
      const prevBuyerBalAfter: BigNumber = await coin.balanceOf(relayer.address)
      const buyerBalAfter: BigNumber = await coin.balanceOf(buyer.address)
      const marketBalAfter: BigNumber = await coin.balanceOf(marketv2.address);
